...
2.  Describe the variables
    ...
    There are 31 variables in the `city_data` object. They include character variables of the city, country and income_id as well as numerical variables (double) that quantify population, waste generated type/percentage of treatment (total of 12 variables about treatment), the composition of the waste (9 variables) and transportation of the waste (4 variables)

3.  Describe the observations

    There are 367 observations. The data is organized by city, where there is one observation for each city.
...
# investigate columns about waste treatment
city_data |> 
  select(starts_with("waste_treatment"))

# investigate columns about composition
city_data |> 
  select(starts_with("composition"))

# investigate columns about transportation
city_data |> 
  select(starts_with("transportation"))

# count columns and categorize observations by city
nrow(city_data)
city_data |> 
  count(unique(city))
...
```{r}
#| label: subset-dist-complete-ext

distance_complete_extended |> 
  filter(1 <= trips_1000_ppl_day, trips_1000_ppl_day < 2)
