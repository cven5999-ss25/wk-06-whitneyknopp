...
# make income_id column into a factor and filter out NA values
income_levels = c("LIC", "LMC", "UMC", "HIC")
treatment_mean_cleaned <- treatment_mean |> 
  mutate(income_id = factor(income_id, levels = income_levels)) |> 
  filter(!is.na(mean))

# plot the data
ggplot(data = treatment_mean_cleaned,
       mapping = aes(x = mean,
                     y = waste_treatment,
                     fill = waste_treatment)) +
  geom_col() +
  facet_wrap( ~ income_id) +
  scale_x_continuous(breaks = seq(0, 40, 5)) +
  scale_fill_brewer(type = "qual", palette = "Paired") +
  labs(title = "Waste Treatment",
       subtitle = "Mean percentages for countries by income region",
       x = "mean (percent)",
       y = "") +
