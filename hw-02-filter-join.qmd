...
1.  How many cities have data for the `pop` variable in the `population` data object? Write code to answer this question.
    ...
    Every city, n = 357, have data for population.
...
3.  Why does the resulting data object have data for 326 countries and not for the identified number of countries with population data in Point 1?
    ...
    This is because we used the `left_join()` function, which keeps all the rows in `generation` (the x data frame) and matches `pop` entries by city from the population data frame (the y)
...
```{r}
#| label: generation-capita

generation_capita <- generation_joined |>
  mutate(msw_generation_capita_kg_year = msw_generation_tons_year / pop * 1000 ) |> 
